...
trigger once while others may trigger infinitely many times. In this case, windfury, divine shield and taunt may only trigger oncle, whilst additional stats
can be rewarded multiple times. Let us call effects that may only trigger once "special effects".
...
if either n-1 or n special effects have been achieved after t-1 buffs. Thus we have that
M(t,n) = M(t-1,n) * prob(next buff is stats) + M(t-1,n) * prob(next buff is a special effect).
